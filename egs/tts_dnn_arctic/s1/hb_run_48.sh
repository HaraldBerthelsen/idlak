...
# Clean up
#HB this means that we always have to run everything from the start so don't do it here but in step0 instead
...
step0=true
step1=true
#HB step 1 only works together with step 0, and the rm line needs to be in use (utt2spk is appended to?)
step2=true
step3a=true
step3b=true
step4a=true
...
#HB some variables need to be moved out of the "if stepX" blocks:
lang=data/lang
expa=exp-align
train=data/full
acdir=data
...
#HB My GPU has only 128M memory, Blaise has 1.5G and uses cache size 50000
dnn_cache_size=5000

#HB
if [ "$step0" == "true" ]; then

#HB not sure if we really want to to this..
rm -rf data/train data/eval data/dev data/train_* data/eval_* data/dev_* data/full
...
#HB export featdir=$TMPDIR/dnn_feats/arctic2
...
#HB tpdb=$KALDI_ROOT/idlak-data/en/ga/
#HB dict=data/local/dict
...
#HB durdir=durdata
#HB lbldurdir=lbldurdata
#HB expdurdir=$exp/tts_dnn_dur_3_delta_quin5
rm -rf $expdurdir
$cuda_cmd $expdurdir/_train_nnet.log steps/train_nnet_basic.sh --delta_order 2 --config conf/3-layer-nn-splice5.conf --learn_rate 0.02 --momentum 0.1 --halving-factor 0.5 --min_iters 15 --max_iters 50 --randomize true --cache_size $dnn_cache_size --bunch_size 200 --mlpOption " " --hid-dim 100 $lbldurdir/train $lbldurdir/dev $durdir/train $durdir/dev $expdurdir

#HB END if [ "$step4a" == "true" ]; then
fi
...
#HB dnndir=$exp/tts_dnn_train_3_deltasc2_quin5
rm -rf $dnndir
$cuda_cmd $dnndir/_train_nnet.log steps/train_nnet_basic.sh --delta_order 2 --config conf/3-layer-nn-splice5.conf --learn_rate 0.04 --momentum 0.1 --halving-factor 0.5 --min_iters 15 --randomize true --cache_size $dnn_cache_size --bunch_size 200 --mlpOption " " --hid-dim 700 $lbldir/train $lbldir/dev $acdir/train $acdir/dev $dnndir


#HB END if [ "$step4b" == "true" ]; then
fi
